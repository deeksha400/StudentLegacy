...
import { parseCertificateText, type ParsedCertificateData } from './lib/verification';
...
function AppShell({ children, user, onLogout }: { children: React.ReactNode; user: UserSession | null; onLogout: () => void }) {
  return (
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-midnight/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                className={({ isActive }) => `text-sm transition ${isActive ? 'text-gold' : 'text-slate-300 hover:text-white'}`}
              >
                {item.label}
              </NavLink>
            ))}
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <>
                <div className="rounded-full border border-gold/20 bg-gold/10 px-3 py-2 text-sm text-gold">
                  {user.name} • {user.role}
                </div>
                <button onClick={onLogout} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm">
                  Log out
                </button>
              </>
            ) : (
              <Link to="/auth" className="rounded-full border border-gold/40 bg-gold/10 px-4 py-2 text-sm font-medium text-gold">
                Login / Register
              </Link>
            )}
          </div>
...
function UploadPage() {
  ...
  const [result, setResult] = useState<ParsedCertificateData | null>(null);
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('text/') || file.name.endsWith('.txt') || file.name.endsWith('.json')) {
      ...
      setRawText(text);
      setResult(parseCertificateText(text, file.name));
    } else {
      setRawText(`Certificate upload detected: ${file.name}`);
      setResult(parseCertificateText(`Certificate upload detected: ${file.name}`, file.name));
    }
    setIsParsing(false);
  };
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-6 backdrop-blur-xl">
          ...
          <div className="mt-6 rounded-3xl border border-dashed border-gold/40 bg-gold/10 p-8 text-center">
            ...
            <p className="mt-2 text-sm text-slate-400">AI extracts student name, organization, title, date, category, and skills with a verification score.</p>
            <label className="mt-6 inline-flex cursor-pointer rounded-full bg-gold px-4 py-2 font-semibold text-slate-950">
              Choose file
              <input type="file" accept=".pdf,.png,.jpg,.jpeg,.txt,.json" className="hidden" onChange={handleUpload} />
            </label>
          </div>
          <div className="mt-6 rounded-2xl border border-white/10 bg-slate-950/70 p-4">
            <label className="text-sm text-slate-400">Certificate notes</label>
            <textarea
              value={rawText}
              onChange={(event) => setRawText(event.target.value)}
              rows={6}
              placeholder="Paste certificate metadata or OCR text here..."
              className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3 py-3 text-white outline-none"
            />
            <button
              onClick={() => setResult(parseCertificateText(rawText || selectedFileName, selectedFileName || 'sample-cert.txt'))}
              className="mt-4 rounded-full bg-gold px-4 py-2 font-semibold text-slate-950"
            >
              Run AI verification
            </button>
          </div>
          {selectedFileName ? <p className="mt-3 text-sm text-slate-400">Selected file: {selectedFileName}</p> : null}
        </div>
        <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-6">
          <div className="flex items-center gap-3 text-gold">
          ...
          {isParsing ? <p className="mt-4 text-sm text-slate-300">Parsing certificate data…</p> : null}
          {result ? (
            <div className="mt-6 space-y-4">
              <div className="rounded-2xl border border-green-500/20 bg-green-500/10 p-4">
                <p className="text-sm text-green-300">Certificate Authenticity Score</p>
                <p className="text-3xl font-semibold text-white">{result.authenticityScore}% Verified</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-400">
              Upload a document to see extracted certificate details and a confidence-based verification score.
            </div>
          )}
        </div>
      </div>
...
function AuthPage({ onAuthenticate }: { onAuthenticate: (user: UserSession) => void }) {
  const [mode, setMode] = useState<AuthMode>('login');
  const [role, setRole] = useState<Role>('student');
  const [name, setName] = useState('Deeksha');
  const [email, setEmail] = useState('deeksha@credvault.edu');
  const [password, setPassword] = useState('');
  const navigate = useNavigate();

  const handleSubmit = (event: FormEvent) => {
    event.preventDefault();
    onAuthenticate({ name, email, role });
    navigate('/dashboard');
  };
    ...
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-6 backdrop-blur-xl">
          ...
          <h1 className="mt-2 text-3xl font-semibold text-white">Role-based authentication</h1>
          <p className="mt-4 text-slate-300">Switch between student, faculty, or admin experiences and sign in instantly with JWT-ready credentials.</p>
          <div className="mt-6 flex flex-wrap gap-2">
            {(['student', 'faculty', 'admin'] as Role[]).map((option) => (
              <button
                key={option}
                onClick={() => setRole(option)}
                className={`rounded-full px-3 py-2 text-sm capitalize ${role === option ? 'bg-gold text-slate-950' : 'border border-white/10 bg-white/5 text-slate-200'}`}
              >
                {option}
              </button>
            ))}
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-6">
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={() => setMode('login')} className={`rounded-full px-3 py-2 text-sm ${mode === 'login' ? 'bg-gold text-slate-950' : 'border border-white/10 bg-white/5 text-slate-200'}`}>
              Login
            </button>
            <button onClick={() => setMode('register')} className={`rounded-full px-3 py-2 text-sm ${mode === 'register' ? 'bg-gold text-slate-950' : 'border border-white/10 bg-white/5 text-slate-200'}`}>
              Register
            </button>
          </div>
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3 py-3 text-white outline-none" placeholder="Your full name" />
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <label className="text-sm text-slate-400">Email</label>
              <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3 py-3 text-white outline-none" placeholder="student@university.edu" />
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <label className="text-sm text-slate-400">Password</label>
              <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3 py-3 text-white outline-none" placeholder="••••••••" />
            </div>
            <button className="w-full rounded-full bg-gold px-4 py-3 font-semibold text-slate-950">{mode === 'login' ? 'Continue' : 'Create account'}</button>
          </form>
        </div>
      </div>
    </section>
...
function CrediVaultApp() {
  const [user, setUser] = useState<UserSession | null>(() => {
    const stored = localStorage.getItem('credvault-user');
    return stored ? JSON.parse(stored) : null;
  });

  useEffect(() => {
    if (user) {
      localStorage.setItem('credvault-user', JSON.stringify(user));
    } else {
      localStorage.removeItem('credvault-user');
    }
  }, [user]);

  const handleAuthenticate = (session: UserSession) => {
    setUser(session);
  };
